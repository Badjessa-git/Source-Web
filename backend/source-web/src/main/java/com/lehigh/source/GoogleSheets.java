...
public class GoogleSheets {
        ...
        try {
            ...
            if (values == null || values.isEmpty()) {
                ...
                for (List row : values) {
                    ...
                    PrintJobRes curJob = new PrintJobRes(jobId, firstName, lastName, club, timeStamp, file, email,
                            colors, numCopies, done);
    ...
    public synchronized List<AllRequestRes> getAllRequest(final String sheetId) {
        final String range = "Form Responses 1";
        try {
            ValueRange response = createService().spreadsheets().values().get(sheetId, range).execute();
            ...
            if (values == null || values.isEmpty()) {
                ...
                for (List row : values) {
                    ...
                    AllRequestRes curReq = new AllRequestRes(jobId, timeStamp, firstName, lastName, club, date,
                            resource);
                    count++;
                    res.add(curReq);
                }
    ...
    public synchronized List<Worker> allWorkers(final String sheetId) {
        final String range = "Form Responses 1";
        try {
            ValueRange response = createService().spreadsheets().values().get(sheetId, range).execute();
            List<List<Object>> values = response.getValues();
            ...
            if (values == null || values.isEmpty()) {
                ...
                for (List row : values) {
                    ...
                    String classYear = (String) row.get(4);
                    String descript = (String) row.get(5);
                    String imgUrl = (String) row.get(6);
                    String email = (String) row.get(7);
                    Worker curWorker = new Worker(firstName, lastName, classYear, major, descript, imgUrl, email);
                    res.add(curWorker);
                    //add to List here
                }
                res.remove(0);
    ...
    /**
     * Update the spreadsheet with the values that they are looking for
     * @param sheetId
     * @param itemId
     * @param newValue
     * @return a value true or false if the update was succesful or not
     */
	public synchronized Boolean updateComplete(String sheetId, String itemId, String newValue) {
        int value = Integer.valueOf(itemId);
        final String range = "Form Responses 1!I"+value;
        ...
        try {
            ...
            UpdateValuesResponse result = createService().spreadsheets().values().update(sheetId, range, body)
                                                        .setValueInputOption("RAW")
                                                        .execute();
            //Check to make sure that the data is updated succesffuly
            if (result.getUpdatedData().equals(body)) {
                return true;
            } else {
                return false;
            }
    ...
    /**
     * Split the string of links thats given into a list of links which is then
     * returned
     * 
